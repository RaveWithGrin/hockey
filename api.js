...
export default function(){
    ...
    var teams = {
        ...
        nextGame: async function(teamId){
            return await callApi('teams/' + teamId + '?expand=team.schedule.next');
        },
        prevGame: async function(teamId){
            return await callApi('teams/' + teamId + '?expand=team.schedule.previous');
        },
    ...
    var player = {
        ...
        seasonStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=statsSingleSeason&season=' + season);
        },
        homeAwayStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=homeAndAway&season=' + season);
        },
        monthlyStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=byMonth&season=' + season);
        },
        dailyStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=byDayOfWeek&season=' + season);
        },
        divisionStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=vsDivision&season=' + season);
        },
        conferenceStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=vsConference&season=' + season);
        },
        teamStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=vsTeam&season=' + season);
        },
        gameStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=gameLog&season=' + season);
        },
        regularSeasonStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=regularSeasonStatRankings&season=' + season);
        },
        goalSituationStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=goalByGameSituation&season=' + season);
        },
        onPaceStats = async function(playerId, season){
            return await callApi('people/' + playerId + '?stats=onPaceRegularSeason&season=' + season);
        }
    };
    ...
    var schedule = {
        day: async function(date, teamId){
            var param = 'schedule?expand=schedule.broadcasts&expand=schedule.linescore&expand=schedule.ticket&date=' + date;
            if (teamId)
                param += '&teamId=' + teamId;
            return await callApi(param);
        },
        range: async function(startDate, endDate, teamId){
            var param = 'schedule?expand=schedule.broadcasts&expand=schedule.linescore&expand=schedule.ticket&startDate=' + startDate + '&endDate=' + endDate;
            if (teamId)
                param += '&teamId=' + teamId;
            return await callApi(param);
        }
    ...
    var standings = {
        season: async function(season){
            return await callApi('standings?season=' +season + '&expand=standings.record');
        },
        date: async function(data){
            return await callApi('standings?date=' + date + '&expand=standings.record');
        }
    };
